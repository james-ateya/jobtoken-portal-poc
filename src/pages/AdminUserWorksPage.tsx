...
export function AdminUserWorksPage({
  ...
  const [tab, setTab] = useState<"prompts" | "jobs">("prompts");
  ...
  const loadApplications = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/admin/users/${userId}/applications?${params}`);
      const json = await res.json().catch(() => ({}));
      ...
      setAppsTotal(Number(json.total) || 0);
      setAppsTotalPages(Math.max(1, Number(json.totalPages) || 1));
    } catch (error: any) {
      showToast(error.message || "Could not load applications", "error");
    } finally {
      setAppsLoading(false);
    }
  }, [userId, appsPage, showToast]);
  ...
  useEffect(() => {
    if (tab === "jobs") void loadApplications();
    ...
    <main className="max-w-5xl mx-auto px-6 py-12 text-white">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">Seeker activity</h1>
            <p className="text-zinc-500 text-sm mt-0.5">
              Prompt reviews (with admin pass/fail notes) and job applications — for support follow-ups.
            </p>
          </div>
        </div>
        <Link
      ...
      {loading && !report ? (
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
              ...
              <div className="rounded-xl border border-white/10 bg-black/20 px-3 py-2">
                ...
                <p className="text-xl font-bold tabular-nums text-sky-300">
                  {tab === "jobs" || appsTotal > 0 ? appsTotal : "—"}
                </p>
              </div>
            </div>
          </section>
          ...
          {tab === "prompts" ? (
                        ...
                        >
                          ...
                          {attempt.grading_note ? (
                            ...
                            <p className="text-xs text-zinc-600 mt-2">
                              No admin feedback note on file.
                            </p>
                          )}
                        </button>
                        ...
                        {open ? (
                          <div className="px-4 pb-4 space-y-3 border-t border-white/5 pt-3">
                            ...
                            <div className="rounded-xl border border-white/10 bg-black/30 p-3">
                              <div className="flex items-center justify-between gap-2 mb-2">
                                <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">
                                  Admin response ({attempt.grade_status})
                                </p>
                                {attempt.grading_note ? (
                                  <button
                                    type="button"
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-10 text-center text-zinc-500">
              No job applications for this user.
            </div>
          ) : (
            <>
              <div className="space-y-3">
                  ...
                  >
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-zinc-500 mt-1">
                          {app.employer_name || "Employer"}
                          {app.employer_email ? ` · ${app.employer_email}` : ""}
                          {app.job_type ? ` · ${app.job_type}` : ""}
                          {app.token_cost != null ? ` · ${app.token_cost} tokens` : ""}
                        </p>
                        <p className="text-xs text-zinc-500 mt-1">
                          Applied {formatDateTime(app.applied_at)}
                          {app.updated_at ? ` · Updated ${formatDateTime(app.updated_at)}` : ""}
                        </p>
                        {app.notes ? (
                      ...
                      <span className="inline-flex items-center px-2 py-1 rounded border border-white/15 bg-white/5 text-[10px] font-bold uppercase tracking-wider text-zinc-300 shrink-0">
        ...
        <div className="text-center text-zinc-500 py-16">User activity could not be loaded.</div>
      )}
